...
export default async function BookingCalendarPage({ searchParams }: Props) {
  ...
  const reservations = selected ? (byDay.get(selected) ?? []).flatMap((slot) => slot.reservations.map((reservation) => ({ reservation, slot }))) : [];
