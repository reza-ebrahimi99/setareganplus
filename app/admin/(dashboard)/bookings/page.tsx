...
const items = [
  ...
  ["📥 خروجی اکسل رزروها", "ساخت گزارش مدیریتی حرفه‌ای از رزروها با فیلتر دلخواه", "/admin/bookings/export"],
] as const;

export default async function BookingsPage() {
  const session = await requirePermission("booking.view_all");
  const visible = hasPermission(session, "settings.manage")
    ? items
    : items.filter((item) => item[2] === "/admin/bookings/calendar" || item[2] === "/admin/bookings/export");
