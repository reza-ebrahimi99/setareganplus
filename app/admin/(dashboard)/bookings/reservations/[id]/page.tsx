...
export default async function BookingReservationPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params; const session = await requirePermission("booking.view_all");
  const reservation = await prisma.bookingReservation.findFirst({ where: { id, organizationId: session.organization.id, deletedAt: null, slot: session.membership.allBranches ? {} : { branchId: { in: session.membership.branchIds } } }, include: { slot: { include: { service: true, advisor: true } }, checkIns: true } });
