...
import { CaseBookDocument } from "@/components/counselor-os/CaseBookDocument";
import { SessionWorksheetDocument } from "@/components/counselor-os/SessionWorksheetDocument";
...
export default async function CounselorExportPrintPage({ params }: Props) {
  ...
  if (kind === "worksheet") {
    const purpose = "جلسه آزاد";
    return (
      <SessionWorksheetDocument
        studentName={bundle.caseModel.studentName}
        counselorName={bundle.caseModel.assignedCounselor?.name ?? "—"}
        sessionLabel={purpose}
      />
    );
  }
  ...
  if (kind === "case") {
    ...
    return (
      <CaseBookDocument
        ...
        late={late}
        counselorName={bundle.caseModel.assignedCounselor?.name ?? undefined}
        ...
        sessions={sessions.map((row) => ({
          title: row.subject || row.sessionType,
          whenLabel: row.scheduledLabel,
          statusLabel: row.status,
          notes: row.summary,
        }))}
      />
