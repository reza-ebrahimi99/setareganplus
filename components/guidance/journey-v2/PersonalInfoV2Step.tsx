...
export function PersonalInfoV2Step(props: {
  ...
  const prefill = useMemo(
    () => stripSyntheticPrefill(props.prefill),
    [props.prefill],
  );
  const isHistorical = props.sidebarSteps.some(
    (step) => step.id === 1 && step.status === "completed",
  ) && props.sidebarSteps.some(
    (step) => step.status === "active" && step.id !== 1,
  );

  const [dob, setDob] = useState<JalaliBirthDateParts>(() =>
    parts(prefill.birthDateJalali ?? ""),
  );
        ...
        <div className="gjv2-form-grid">
          ...
          <div className="gjv2-form-field">
            ...
            <input
              ...
              defaultValue={prefill.alternateMobile ?? ""}
              inputMode="tel"
          ...
          <Select
            label="استان بومی"
            name="nativeProvince"
            defaultValue={prefill.nativeProvince}
          ...
          <Select
            label="سهمیه مناطق"
            name="regionQuota"
            defaultValue={prefill.regionQuota}
            error={e.regionQuota}
          ...
          <Select
            label="سهمیه خاص"
            name="specialQuota"
            defaultValue={prefill.specialQuota || "NORMAL"}
