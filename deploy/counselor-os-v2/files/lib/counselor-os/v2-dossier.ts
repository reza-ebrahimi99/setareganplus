...
export async function loadCounselorV2Dossier(params: {
  ...
  if (!plan) {
    return {
      ...
      examGroups: null,
      grades: [],
      holland: { completed: false, codes: [], summary: null },
      educationTypes: [],
      provinces: [],
      majors: [],
      priorities: [],
  ...
  const hollandRaw = asRecord(holland.data);
  const hollandResult = asRecord(hollandRaw?.result);
  const hollandCodes = Array.isArray(hollandResult?.codes)
    ? hollandResult.codes.map(String)
    : hollandResult?.threeLetterCode
      ? [String(hollandResult.threeLetterCode)]
      : [];
  ...
  const fieldSets: Record<number, Array<{ label: string; value: string }>> = {
    ...
    4: [
      {
        label: "وضعیت آزمون",
        value: hollandRaw ? "تکمیل شده" : "شروع نشده",
      },
    ],
    5: [
      {
  ...
  return {
    ...
    grades: gradeRows,
    holland: {
      completed: Boolean(hollandRaw),
      codes: hollandCodes,
      summary: hollandCodes.length
        ? `الگوی رغبت: ${hollandCodes.join(" · ")}`
        : null,
    },
    educationTypes,
    provinces,
