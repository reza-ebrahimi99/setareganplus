...
export async function saveCounselorSchedule(input: {
  ...
  await refreshCounselorGeneratedSlots({
    organizationId: input.ctx.organizationId,
    advisorId: advisor.id,
    validFromYmd: input.validFromYmd,
    validUntilYmd: input.validUntilYmd,
  });
}
...
export async function refreshCounselorGeneratedSlots(params: {
  ...
  const now = new Date();
  const fromBound = ymdToUtcNoon(params.validFromYmd) ?? now;
  const untilBound =
    ymdToUtcNoon(params.validUntilYmd) ??
  ...
  const from = utcToJalaliInTehran(rangeStart);
  const to = utcToJalaliInTehran(rangeEnd);
  ...
  await generateSlotsForRange({
    ...
    from,
    to,
    durationMinutes: durations.firstSessionMinutes,
  ...
  await generateSlotsForRange({
    ...
    from,
    to,
    durationMinutes: durations.secondSessionMinutes,
    exceptionWindows: "append",
  });
