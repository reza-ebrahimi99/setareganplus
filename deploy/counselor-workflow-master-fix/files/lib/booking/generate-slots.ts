...
export type GenerateSlotsInput = {
  ...
  dryRun?: boolean;
  /**
   * Override service.durationMinutes for this run (FIRST vs SECOND session).
   * Public booking omits this and keeps the service default.
   */
  durationMinutes?: number;
  /**
   * `replace` (default): a special-date exception replaces weekly windows.
   * `append`: special-date windows are added on top of weekly windows.
   */
  exceptionWindows?: "replace" | "append";
  /**
   * When set, weekly DB rules are ignored and these windows are used instead.
   * FIRST vs SECOND counselor schedules must pass independent window sets.
   */
  windowsOverride?: Array<{
    weekday: number;
...
export async function generateSlotsForRange(
  ...
  const rules = input.windowsOverride
    ? input.windowsOverride.map((window, index) => ({
        id: `override-${index}`,
        weekday: window.weekday,
        startLocalTime: window.startLocalTime,
        endLocalTime: window.endLocalTime,
        slotCapacity: window.slotCapacity ?? 1,
        validFrom: window.validFrom ?? null,
        validUntil: window.validUntil ?? null,
      }))
    : await prisma.bookingAvailabilityRule.findMany({
        where: {
          organizationId: input.organizationId,
  ...
  const stepMinutes =
    durationMinutes + service.bufferBeforeMinutes + service.bufferAfterMinutes;
  const exceptionWindows = input.exceptionWindows ?? "replace";
  if (stepMinutes <= 0) {
    return {
      created: 0,
      existing: 0,
  ...
  for (const day of eachJalaliDay(input.from, input.to)) {
    ...
    if (specialWindow && exceptionWindows === "replace") {
      windows.push(specialWindow);
    } else {
      ...
      if (specialWindow && exceptionWindows === "append") {
        windows.push(specialWindow);
      }
    }
    ...
    for (const window of windows) {
      ...
      while (cursorMinutes + durationMinutes <= endMinutes) {
        ...
        const found = await prisma.bookingSlot.findUnique({
          ...
          select: { id: true, status: true, endsAt: true, bookedCount: true },
        });

        if (found) {
          existing += 1;
          const existingDuration = Math.round(
            (found.endsAt.getTime() - startsAt.getTime()) / 60000,
          );
          const sameDuration = existingDuration === durationMinutes;
          if (
            !dryRun &&
            found.status !== BookingSlotStatus.CANCELLED &&
            found.bookedCount === 0 &&
            sameDuration
          ) {
