...
#
# ==============================================================================
# Discover detail-page cleanup — production applier
# ==============================================================================
#
# Presentation only. Replaces the legacy university-system detail article +
# school-photo cover with the encyclopedia template, and MERGES the DISCOVER
# DETAIL CLEANUP CSS block into app/globals.css. Never replaces globals.css
# wholesale.
#
# NEVER:
...
BACKUP="/var/backups/discover-detail-cleanup-${STAMP}"
...
FORBIDDEN_RX='(^|/)(prisma/|generated/)|counselor|lib/guidance/journey-v2|zibal|Zibal|middleware\.ts|\.env|sms|Sms|SMS|PackagePayment|step10-'
...
[[ -d "$BUNDLE" ]] || die "bundle missing: $BUNDLE (copy deploy/discover-detail-cleanup onto the server)"
...
ok "data sources, payment libs, middleware, and Prisma are not in the allowlist"
...
require_needles "${BUNDLE}/components/guidance/discover/ProgramEncyclopediaDetail.tsx" "bundle" \
  ...
  "item.suitableFor" \
  "ProgramAtAGlance"
ok "bundled Discover files have the expected presentation contracts"
...
#!/usr/bin/env bash
# Restore every file this Discover cleanup run touched, then rebuild and restart.
set -euo pipefail
cd "${APP_ROOT}"
...
# ==============================================================================
step "3. Copy Discover detail presentation files"
...
smoke() {
  ...
  code="$(curl -sS -o /tmp/discover-detail-cleanup-smoke.body -w '%{http_code}' \
...
smoke "${BASE}/discover/systems/daily"
smoke "${BASE}/discover/programs"
smoke "${BASE}/discover/programs/payam-noor"
smoke "${BASE}/discover/programs/bachelor-continuous"
smoke "${BASE}/portal/student/services/guidance"
smoke "${BASE}/entekhab"

ok "Discover detail routes responded without 5xx"

trap - ERR

echo
echo "=== DISCOVER DETAIL CLEANUP DEPLOYED ==="
echo "backup: ${BACKUP}"
echo "typecheck: PASS"
echo "build: PASS"
