...
import { assertArrangementEntitlement } from "@/lib/guidance/journey-v2/entitlements";
...
async function requirePaidStep(
  step: GuidanceV2StepId,
  mode: "current-only" | "editable" = "current-only",
) {
  const access = await requireGuidanceV2StepAccess(step);
  if (mode === "current-only" && access.plan.currentStep !== step) {
    redirect(guidanceJourneyV2StepPath(access.plan.currentStep));
  }
  const arrangement = assertArrangementEntitlement({
    packageCode: access.plan.guidancePackageCode,
    stepId: step,
  });
  if (!arrangement.ok) {
...
export async function bookV2SessionAction(
  ...
  const step = Number(field(formData, "step"));
  if (step !== 11 && step !== 15) {
    return { error: "مرحله رزرو نامعتبر است." };
  }
  const purpose =
  ...
  const completed = await completeBookedV2Step({
    step,
    purpose,
    organizationId: access.context.organization.id,
...
async function completeBookedV2Step(params: {
  step: 11 | 15;
  purpose: typeof APPOINTMENT_PURPOSE.FIRST_SESSION | typeof APPOINTMENT_PURPOSE.SECOND_SESSION;
  ...
  if (!counselor) {
    return { error: "پرونده شما در حال تخصیص به مشاور است." };
  }
  const qualified = await loadQualifyingSessionBooking({
    organizationId: params.organizationId,
    studentId: params.studentId,
    purpose: params.purpose,
    planId: params.planId,
    counselorUserId: counselor.counselorUserId,
  });
  if (!qualified.ok) return { error: qualified.error };
  ...
  const advanced = await advanceGuidanceJourneyV2Step({
    ...
    studentId: params.studentId,
    stepId: params.step,
    metadata: { appointmentId: qualified.appointmentId, purpose: params.purpose },
  });
...
export async function submitKonkurResultAction(
  ...
  const access = await requirePaidStep(12, "editable");
  const unpaid = packageGuard(access.plan);
  if (unpaid) return unpaid;
  ...
  const latestExam = await prisma.guidanceDocument.findFirst({
    where: {
  ...
  const docGate = assertRequiredDocumentApproved({
    stepId: 12,
    hasLatest: Boolean(latestExam),
    latestStatus: latestExam?.verificationStatus,
  });
  if (!docGate.ok) return { error: docGate.error };
...
export async function saveChoiceFeedbackAction(
  ...
  const access = await requirePaidStep(14, "editable");
  try {
