...
import { resolveGuidanceV2StepAccessMode } from "@/lib/guidance/journey-v2/step-access-policy";
...
export async function renderGuidanceV2LateStep(step: GuidanceV2StepId) {
  ...
  const hasArrangement = packageIncludesArrangement(plan.guidancePackageCode);
  const accessMode = resolveGuidanceV2StepAccessMode(step, {
  ...
  const [firstAppt, secondAppt, initialListForSignal, finalListForSignal, sanjeshForSignal] =
    await Promise.all([
      loadPurposeAppointmentView({
      ...
      loadSanjeshCase({ organizationId: orgId, planId: plan.id }),
    ]);

  const timeline = buildLateJourneyTimeline({
    currentStep: plan.currentStep,
    packagePaid,
    hasArrangementEntitlement: hasArrangement,
    firstSessionBooked: Boolean(firstAppt),
    secondSessionBooked: Boolean(secondAppt),
    konkurSubmitted: plan.completedSteps.includes(12) || plan.currentStep > 12,
    initialReady: initialListForSignal?.status === CHOICE_LIST_STATUS.READY,
    reviewComplete: plan.completedSteps.includes(14) || plan.currentStep > 14,
    finalReady: finalListForSignal?.status === CHOICE_LIST_STATUS.READY,
    informedConfirmed: Boolean(plan.v2InformedRevisionId || plan.finalApprovedAtIso),
    sanjeshDeclared: Boolean(sanjeshForSignal.declaredAtLabel || plan.v2SanjeshDeclaredAtIso),
    sanjeshVerified: sanjeshForSignal.status === SANJESH_STATUS.VERIFIED,
  ...
  if (!packagePaid) {
  ...
  } else if (!hasArrangement && step >= 13) {
    body = (
      <section className="gv2-status-card gv2-status-card--premium">
        <p className="gv2-status-card__eyebrow">پلن رایگان</p>
        <h2>چیدمان مشاور در پلن شروع فعال نیست</h2>
        <p>
          پلن رایگان تا ثبت اولویت رشته، شهر و دوره ادامه دارد. برای چیدمان ۱۵۰
          انتخاب و همراهی مشاور یکی از پلن‌های هوشمند، تخصصی یا ممتاز را تهیه کنید.
        </p>
        <a className="gv2-late-nav__link gv2-late-nav__link--pay" href={paymentHref}>
          مشاهده پلن‌های انتخاب رشته
        </a>
      </section>
    );
  } else if (accessMode === "locked") {
    body = (
      <LateStepLookBackNotice
        currentStep={plan.currentStep}
    ...
    body = (
      <KonkurResultForm
        ...
        documentStatus={konkur.document?.verificationStatus ?? null}
        documentNote={konkur.document?.reviewNote ?? null}
      />
    );
  } else if (step === 13) {
    ...
    body = (
      <WaitingStatusCard
        ...
        body={`${CHOICE_STUDIO_BRAND}. چیدمان نسخه اولیه فقط توسط مشاور انجام می‌شود و به معنی ثبت سنجش نیست. پس از انتشار نسخه اولیه، مرحله بررسی برای شما باز می‌شود.`}
        meta={
          list
            ? [
                {
                  label: "تعداد انتخاب فعال",
                  value: String(list.items.filter((i) => i.isActive).length),
                },
                { label: "وضعیت", value: labelArrangementState(stateKey) },
              ]
            : undefined
        }
      />
    ...
    if (!list || list.status !== CHOICE_LIST_STATUS.READY) {
      body = (
        <WaitingStatusCard
          ...
          body="نسخه اولیه چیدمان هنوز از سمت مشاور منتشر نشده است."
        />
      );
    } else {
      const [stats, counselor] = await Promise.all([
        reviewStats(list),
        loadAssignedCounselorForStudent({ organizationId: orgId, studentId }),
      ]);
      body = (
        <ChoiceReviewWorkspace
          ...
          studentName={`${context.user.firstName} ${context.user.lastName}`.trim()}
          counselorName={counselor?.name}
          readyAtLabel={
            list.readyAtIso ? formatJalaliDateTimeShort(new Date(list.readyAtIso)) : null
          }
        />
      );
    }
    ...
    if (!list || list.status !== CHOICE_LIST_STATUS.READY) {
      ...
      const initialList = await loadChoiceListView({
        organizationId: orgId,
        planId: plan.id,
        kind: CHOICE_LIST_KIND.INITIAL,
      });
      body = (
        <InformedConfirmPanel
          list={list}
  ...
  return (
    <LateStepShell
      ...
      timeline={timeline}
    >
      {!isCurrent && accessMode === "edit" ? <LateStepEditWarning /> : null}
      {body}
