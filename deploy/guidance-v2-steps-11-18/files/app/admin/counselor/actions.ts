...
export async function saveCounselorScheduleAction(
  ...
  try {
    await saveCounselorSchedule({
      ...
      validFromYmd: field(formData, "validFrom"),
      validUntilYmd: field(formData, "validUntil"),
      days: parseWeeklyProgramJson(field(formData, "windowsJson")),
    });
  } catch (e) {
...
export async function deleteCounselorExceptionAction(
  ...
  try {
    await deleteCounselorDateException({ ctx, exceptionId });
  } catch (e) {
