...
function Letterhead(props: { title: string; studentName: string }) {
  return (
    <>
...
export function LatePrintDocument(props: {
  ...
  if (props.kind === "choices-initial") {
    return (
      <div className="counselor-report" dir="rtl">
        <Letterhead title="چیدمان اولیه مشاور" studentName={name} />
  ...
  if (props.kind === "choices-final") {
    return (
      <div className="counselor-report counselor-choice-final" dir="rtl">
        <Letterhead title="فهرست نهایی انتخاب رشته" studentName={name} />
        <p>
          این فهرست برای ورود دستی در سامانه رسمی سازمان سنجش تهیه شده است. کد رسمی فقط در صورت وجود
          مقدار واقعی چاپ می‌شود.
        </p>
        <ChoiceTable list={props.late.finalMapped.list} />
