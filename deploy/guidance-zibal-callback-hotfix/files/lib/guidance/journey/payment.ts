...
export async function startGuidancePackageCheckout(params: {
  ...
  const idempotencyKey = quoted.discountCode
    ? `guidance-package:${plan.id}:${pkg.priceRials}:d:${quoted.discountCode}`
    : `guidance-package:${plan.id}:${pkg.priceRials}`;
  ...
  if (!intent) {
    intent = await prisma.paymentIntent.create({
      data: {
        ...
        discountRials: quoted.discountRials,
        finalAmountRials: quoted.finalAmountRials,
        metadata: quoted.discountCode
          ? {
              guidanceDiscountCode: quoted.discountCode,
              guidanceDiscountId: quoted.discountId,
            }
          : undefined,
        currency: "IRR",
        description,
    ...
    if (quoted.discountId) {
      const consumed = await consumeGuidanceDiscountUse({
        organizationId: params.organizationId,
        discountId: quoted.discountId,
      });
      if (!consumed) {
  ...
  if (provider.id === "zibal") {
    ...
    if (
      ...
      if (consumedNow && quoted.discountId) {
  ...
  if (nextStatus === PaymentStatus.FAILED || nextStatus === PaymentStatus.CANCELLED) {
    const meta =
      intent.metadata && typeof intent.metadata === "object" && !Array.isArray(intent.metadata)
        ...
        : {};
    if (typeof meta.guidanceDiscountId === "string" && meta.guidanceDiscountId) {
      await releaseGuidanceDiscountUse({
        organizationId: params.organizationId,
        discountId: meta.guidanceDiscountId,
      });
    }
  }
