...
/**
 * Resolve the Zibal return URL.
 * An explicit caller callbackPath always wins. ZIBAL_CALLBACK_URL is only
 * the fallback when no dedicated path was supplied (legacy/general flows).
 */
export function buildZibalCallbackUrl(input: {
  callbackPath: string;
  callbackToken: string;
}): string | null {
  const path = input.callbackPath.trim();
  ...
  if (path.length > 0) {
    if (/^https?:\/\//i.test(path)) {
      ...
      base = `${PUBLIC_SITE_ORIGIN}${normalized}`;
    }
  } else if (configured) {
    base = configured;
  }
...
export function isZibalCallbackForPath(
  callbackUrl: string,
  expectedPath: string,
): boolean {
  try {
...
export class ZibalPaymentProvider implements PaymentProvider {
  ...
  async requestPayment(
    ...
    const callbackUrl = buildZibalCallbackUrl(input);
    if (!callbackUrl) {
