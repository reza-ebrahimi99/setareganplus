...
export async function ensureCounselorProfile(params: {
  ...
  const unlinked = await prisma.bookingAdvisor.findMany({
    where: {
      organizationId: params.organizationId,
      userId: null,
      deletedAt: null,
...
/** Repair missing User ↔ BookingAdvisor links for ADVISOR staff. Never creates duplicates. */
export async function ensureOrgCounselorAdvisorLinks(organizationId: string) {
  ...
  for (const membership of memberships) {
    if (membership.user.deletedAt) continue;
    await ensureCounselorProfile({
      organizationId,
      userId: membership.userId,
      displayName: displayName(membership.user.firstName, membership.user.lastName),
    });
  }
}
