...
export function slotMatchesProgram(params: {
  startMinutes: number;
  durationMinutes: number;
  weekday: number;
  ymd: string;
  days?: WeekdayWindows[] | null;
  fromYmd?: string | null;
  untilYmd?: string | null;
}): boolean {
  if (params.fromYmd && params.ymd < params.fromYmd) return false;
  if (params.untilYmd && params.ymd > params.untilYmd) return false;
  if (!params.days || params.days.length === 0) return true;
  const day = params.days.find((item) => item.weekday === params.weekday);
  if (!day?.enabled) return false;
  return slotFitsSingleWindow({
