...
export function counselorChoiceMutationLock(params: {
  kind?: string | null;
  status?: string | null;
}): "editable" | "published_snapshot" | "confirmed" {
  if (params.status === CHOICE_LIST_STATUS.CONFIRMED) return "confirmed";
  if (params.status === CHOICE_LIST_STATUS.READY) return "published_snapshot";
  return "editable";
}
