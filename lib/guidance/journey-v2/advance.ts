...
export async function advanceGuidanceJourneyV2Step(params: {
  ...
  if (!arrangement.ok) {
    return { ok: false, error: arrangement.error };
  }

  if (params.stepId === 3 || params.stepId === 12) {
    const documentType = params.stepId === 3 ? "FINAL_GRADES" : "EXAM_RESULT";
    const latest = await prisma.guidanceDocument.findFirst({
      where: {
        organizationId: params.organizationId,
        planId: plan.id,
        documentType,
    ...
    const gate = assertRequiredDocumentApproved({
      stepId: params.stepId,
      hasLatest: Boolean(latest),
      latestStatus: latest?.verificationStatus,
    });
    if (!gate.ok) return { ok: false, error: gate.error };
  }

  if (params.stepId === 11 || params.stepId === 15) {
    const purpose =
      params.stepId === 15
        ? APPOINTMENT_PURPOSE.SECOND_SESSION
        : APPOINTMENT_PURPOSE.FIRST_SESSION;
    ...
    if (!qualified.ok) {
      return { ok: false, error: qualified.error };
    }
  }
