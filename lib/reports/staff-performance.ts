...
export async function loadStaffPerformance(
    ...
    ...(branchIds
      ? {
          OR: [
            ...
            {
              branchMemberships: {
                some: { branchId: { in: branchIds }, deletedAt: null },
              },
            },
          ],
        }
  ...
  const leadFilter: Prisma.LeadWhereInput = {
    ...
    ...(filters.formId
      ? { formSubmissions: { some: { formId: filters.formId } } }
      : {}),
    ...(filters.stageId ? { stageId: filters.stageId } : {}),
  };
  const range = { gte: filters.from, lte: filters.to };

  const [assignedByOwner, calls, tasks, activities, branches, stages, forms] =
    await Promise.all([
      // KPI Engine: assignedLeads ≡ leads created in range, dimension ownerUserId
      runKpiFormula("count_leads_created", {
        ...
        branchIds,
        leadScope,
      }).then((points) => kpiPointsByUserId(points, "ownerUserId")),
      ...
      // KPI source (not PaymentIntent): staff activity metrics stay actor-scoped
      loadCrmActivities({
        organizationId,
        from: filters.from,
        to: filters.to,
      ...
      prisma.branch.findMany({
        where: {
          organizationId,
          deletedAt: null,
          ...(allowedBranches ? { id: { in: allowedBranches } } : {}),
        },
        orderBy: { name: "asc" },
  ...
  const rows = members.map((member) => {
    const memberCalls = calls.filter(
      (call) => call.membershipId === member.id,
    );
    const memberTasks = tasks.filter(
      (task) => task.assignedToUserId === member.user.id,
    );
    const memberActivities = activities.filter(
      (activity) => activity.actorUserId === member.user.id,
    );
    const firstCalls = new Map<string, (typeof memberCalls)[number]>();
    for (const call of memberCalls) {
      if (!firstCalls.has(call.leadId)) firstCalls.set(call.leadId, call);
    }
    const responseTimes = [...firstCalls.values()]
      .map((call) => call.calledAt.getTime() - call.lead.createdAt.getTime())
      .filter((value) => value >= 0);
    // Actor-based won (staff report definition) — not KPI owner-attributed conversion
    const won = memberActivities.filter(
      (activity) => activity.activityType === CrmActivityType.CONVERTED,
    ).length;
    const assignedCount = assignedByOwner.get(member.user.id) ?? 0;
    const activeLeadCount = new Set(
      memberActivities.map((activity) => activity.leadId),
    ).size;
    return {
      ...
      answered: memberCalls.filter((call) =>
        ANSWERED_OUTCOMES.has(call.outcome),
      ).length,
      noAnswer: memberCalls.filter((call) =>
        NO_ANSWER_OUTCOMES.has(call.outcome),
      ).length,
      followUpsCompleted: memberTasks.filter(
        (task) =>
          task.taskType === CrmTaskType.FOLLOW_UP &&
          task.status === CrmTaskStatus.COMPLETED &&
          task.completedAt &&
          task.completedAt >= filters.from &&
          task.completedAt <= filters.to,
      ).length,
      ...
      consultations: memberCalls.filter(
        (call) => call.outcome === CrmCallOutcome.CONSULTATION_BOOKED,
      ).length,
      qualified: memberActivities.filter((activity) => {
        if (
          activity.activityType !== CrmActivityType.STAGE_CHANGED ||
          !activity.metadata ||
        ...
        return (
          (activity.metadata as Record<string, unknown>).stageType ===
          "QUALIFIED"
        );
      }).length,
      won,
