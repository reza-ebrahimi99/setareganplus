#!/usr/bin/env tsx
/**
 * Operator helper: enable/disable the organization-scoped `bookCommerce` flag.
 * Default remains OFF. Does not change STAROS_BOOKS_ERP_HARD_OFF.
 *
 * Usage:
 *   npm run books:set-flag -- --slug=demo --enabled=true
 */
...
import { BOOKS_FEATURE_FLAG_KEY } from "../lib/books/constants";
import { ensureBookAgencyProfile, ensureDefaultBookTypes } from "../lib/books/agency-profile";
...
async function main() {
  ...
  if (enabled) {
    await ensureBookAgencyProfile(organization.id);
    await ensureDefaultBookTypes(organization.id);
  }
