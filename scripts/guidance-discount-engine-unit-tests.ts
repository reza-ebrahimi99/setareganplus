...
test("SPECIALIZED 10% uses approved 57,000,000 rials", () => {
  const calc = calculateGuidanceDiscount({
    originalAmountRials: 57_000_000,
    type: "PERCENTAGE",
    value: 10,
  });
  assert.equal(calc.ok, true);
  if (!calc.ok) return;
  assert.equal(calc.result.originalAmountRials, 57_000_000);
  assert.equal(calc.result.discountAmountRials, 5_700_000);
  assert.equal(calc.result.finalAmountRials, 51_300_000);
  assert.equal(rialToToman(calc.result.originalAmountRials), 5_700_000);
});
